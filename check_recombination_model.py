...
for i in range(len(probes)):
    file_path = probes[i]
    with open(file_path + '.inputparams', 'r') as f:
        input_params = f.readlines()
        input_params = [line.strip() for line in input_params if line.strip() and not line.startswith('#')]
        input_params = {line.split('=')[0].strip(): line.split('=')[1].strip() for line in input_params}
        print(input_params.keys())
        exit()
    rec_model[probes[i]] = input_params.get('Compiled_Recombination', 'unknown')
